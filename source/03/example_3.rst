...
- [1] 日本国内であれば、国土地理院の基盤地図情報（数値標高モデル）から10mメッシュの標高データ（DEM）をダウンロードできます。また、`ASTER GDEM <https://www.jspacesystems.or.jp/ersdac/GDEM/E/1.html>`_ でも全球で30mメッシュ程度の高解像度地形データを入手することができます。
...
申請すれば誰でも利用できる `UC tools <https://tools.i-ric.info/login/>`_ を使うことで、雨量データファイル"rain.dat"を容易に作成することもできます。UC toolsを用いた詳細な手順については、概要の「２．降雨データセットの作成」を参照してください。
...
.. figure:: img_3/rri_demAdjust2_main.jpg
   :scale: 50%
   :alt:

- 座標系：経度・緯度
- DEMファイル：filldem.txt（水文補正標高）
- DIRファイル：表面流向データファイル
- ACCファイル：上流集水グリッド数ファイル

**「河道形状」タブ**

.. figure:: img_3/rri_demAdjust2_river.jpg
   :scale: 50%
   :alt:
...
なお、川幅や水深はRRIモデルの重要なパラメータです。これらを変更する場合には、格子の生成をやり直してください。
...
計算開始後の降雨がない時間帯は、計算時間短縮のため、土砂の解析を行わないよう設定します。「河床変動計算の時間刻み」について、一回のRRIモデルの河道の計算時間間隔（デフォルトは60秒）の間に、土砂の計算を何回行うかを設定します。浮遊砂を扱う場合、時間刻みを細かく（この数字を大きく）した方が計算が安定します。ただし値を大きくするほど計算時間が長くなります。
