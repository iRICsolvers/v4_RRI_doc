...
- 河道が満砂の時に土砂を溢れさせる：土砂・洪水氾濫など、一度に多量の土砂が河道に供給させる場合、単位河道が埋まってしまい、それ以上土砂が下流に輸送されない場合があります。これを避けたい場合にこの機能を有効にすると、土砂の堆積によって河道の容量がなくなりそうな場合に、河道の土砂が氾濫原に溢れたとみなし、河道にそれ以上の土砂が堆積しないように設定することが可能です。
...
6.7. RSRモデルの詳細設定
--------------------------------------------------
ソルバーコンソールに、詳細な計算結果を出力することも可能です。

RSRモデルでは、地形データに基づいて、合流点間を単位とする単位河道を流域全体で自動的に生成します。
そのため、長い区間にわたり合流点が存在しない場合には、一つの単位河道が極めて長くなってしまう場合があります。
これを避けるために、本機能を用いることで、長い単位河道をいくつかの単位河道に分割することが可能です。
分割したい区間の始点と終点の座標(i,j)を指定して、その区間の分割数を設定することで、単位河道を分割することができます。
